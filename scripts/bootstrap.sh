...
if [[ ! -d "$CONFIG_DIR" ]]; then
    echo "[→] CONFIG_DIR '$CONFIG_DIR' does not exist — initializing from git repo..."
    sudo mkdir -p "$CONFIG_DIR"

    echo "[→] Copying contents from '$GIT_REPO_PATH' to '$CONFIG_DIR'..."
    sudo cp -r "$GIT_REPO_PATH"/. "$CONFIG_DIR"/

    echo "[→] Setting ownership to $CURRENT_USER:$GROUP..."
    sudo chown -R "$CURRENT_USER:$GROUP" "$CONFIG_DIR"

    echo "[→] Setting permissions 2775 and setgid bit..."
...
echo "=== [2] Do hardware-configuration.nix stuff ==="
...
echo ""
echo "=== [4] Preparing flake: temporarily adding hardware config to Git index ==="
cd "$FLAKE_DIR"
...
echo ""
echo "=== [5] Bootstrap rebuild with flakes ==="
...
if [[ "$USER_ADDED_TO_GROUP" == true ]]; then
    echo ""
    echo "[i] User '$CURRENT_USER' was added to group '$GROUP'."
    echo "[i] To edit configs immediately, run: newgrp $GROUP"
